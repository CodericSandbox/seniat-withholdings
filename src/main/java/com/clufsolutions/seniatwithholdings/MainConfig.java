...
@Configuration
@ComponentScan(
        basePackages = "com.clufsolutions.seniatwithholdings",
...
public class MainConfig implements CommandLineRunner {
	...
	private void marshallThings() {
		try {
			Marshaller m = JAXBContext.newInstance(XmlWithholding.class,
					XmlDocument.class).createMarshaller();
			m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
			m.marshal(
					new XmlWithholding(w1, w1.getCompany().getTaxes()
							.get("IVA").getAlicuote()), System.out);
    ...
    @Bean
    public ServletRegistrationBean restExporterServletRegistration() {
        AnnotationConfigWebApplicationContext webCtx = new AnnotationConfigWebApplicationContext();
        webCtx.register(RestExporterConfig.class);

        DispatcherServlet restExporterServlet = new DispatcherServlet(webCtx);
        ServletRegistrationBean reg = new ServletRegistrationBean(restExporterServlet);
        reg.setName("restExporterServlet");
